...
class Solution {
    /**
     * 完全背包：有N件物品和一个最多能背重量为W 的背包。第i件物品的重量是weight[i]，得到的价值是value[i] 。每件物品可以用无数次，求解将哪些物品装入背包里物品价值总和最大
     *
     * N = 3, W = 4
     *       重量   价值
     * 物品0	  1	    15
     ...
     * @return
     */
    public int weightBag_CompletePack(int[]weight, int[] value, int W, int N) {
        ...
        //4. 遍历顺序：跟01背包唯一的不同点就是遍历顺序的不一致。在完全背包中，因为每个物品可以放入无数次，因此遍历背包容量的时候是从小到大。并且既可以先遍历背包再遍历物品，也可以先遍历物品再遍历背包
        for (int i = 0; i < weight.length; i++){//遍历物品：weight[0], weight[1], weight[2]
            for (int j = weight[i]; j <= W; j++){//遍历背包：dp[1], dp[2], dp[3], dp[4]
    ...
    public static void main(String[] args) {
        ...
        int[] weight = {1, 3, 4};
        int[] value = {15, 20, 30};
        s.weightBag_CompletePack(weight, value, W, N);
    }
}
